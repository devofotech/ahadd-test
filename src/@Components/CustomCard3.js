...
export default ({
  children, isCreate, adjustStyle = {}, isToTheSide = 3, selected, onClick = () => null, className, hoverColor, noAnimation = false,
}) => {
  const [onHover, setOnHover] = useState(false);
  const onHoverColor = hoverColor ?? 'var(--primary-color)';
  const borderColor = onHover ? onHoverColor : '#C8CBD3';
  const transform = !noAnimation && ((onHover && !isCreate) && 'scale(1.01)');
    ...
    <Grid item xs={isToTheSide}>
      <div
        ...
        style={{
          ...
          border,
          transform,
          transition: !noAnimation ?? 'all .25s',
