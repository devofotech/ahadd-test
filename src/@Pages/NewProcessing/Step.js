...
export default ({ activeStep, isOrgUnlimited }) => {
  const steps = ['Processing creation', 'Image upload', 'Processing output', isOrgUnlimited ? 'Summary' : 'Payment'];
  return (
    <Box sx={{ width: '100%' }}>
      <Stepper activeStep={activeStep} style={{ width: '90%' }} className="bg-transparent mx-auto">
        {steps.map((label) => (
          <Step key={label}>
